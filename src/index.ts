...
InstagramApi.getFollowers(username, password).then(async followers => {
    ...
    if (program.followed)
        console.log("Followed:", followed.map(getFollowerName));

    if (program.unfollowed)
        console.log("Unfollowed:", unfollowed.map(getFollowerName));

    if (program.showFollowers)
        console.log(`Followers (${program.showFollowers}):`, FileSystem.loadFollowers(program.showFollowers).map(getFollowerName));

    if (program.followerList)
        console.log("Complete Followed List:", FileSystem.loadFollowed().map(getFollowerName));

    if (program.unfollowerList)
        console.log("Complete Unfollowed List:", FileSystem.loadUnfollowed().map(getFollowerName));
    ...
    if (program.accountsYouNotFollow) {
        ...
        console.log(
            ...
            FollowerHelper.compareFollowers(followed, followers)[1].map(getFollowerName)
        );
    }
    ...
    if (program.accountsNotFollow) {
        ...
        console.log(
            ...
            FollowerHelper.compareFollowers(followed, followers)[0].map(getFollowerName)
        );
    }
